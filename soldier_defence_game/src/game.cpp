...
bool Game::initialize() 
{
	...
	m_enemy_wave.enemy_spawn(m_texture_cache, m_nr_enemies, m_window_size);
...
void Game::update(float dt)
{
	...
	if (m_gamestate_manager.m_gamestate == m_gamestate_manager.gamestate::playing)
	{
		...
		if (m_enemy_wave.m_wave_number >= m_enemy_wave.m_max_number_of_waves && m_enemy_wave.m_enemies.empty()) { m_gamestate_manager.change_gamestate(m_gamestate_manager.gamestate::win); }
		if (m_player.m_health <= 0) { m_gamestate_manager.change_gamestate(m_gamestate_manager.gamestate::lose); }
...
void Game::draw()
{
	...
	if (m_gamestate_manager.m_gamestate == m_gamestate_manager.gamestate::start) 
	{
		DrawText("Ditch Effort", 20, int(m_window_size.y * 0.40), 50, m_text_colour);
		DrawText("Press S to begin your last stand", 20, int(m_window_size.y * 0.5), 20, m_text_colour);
	}
	if (m_gamestate_manager.m_gamestate == m_gamestate_manager.gamestate::playing) 
	{
		if(m_scoreboard.m_score == 0 ) { DrawText("LEFT CLICK TO SHOOT", int(m_window_size.x * 0.20), int(m_window_size.y * 0.80), 30, m_text_colour); }
	...
	{
		DrawText("You have been rescued by comrades, congratulations!", 20, int(m_window_size.y * 0.5), 20, m_text_colour);
	}
	if (m_gamestate_manager.m_gamestate == m_gamestate_manager.gamestate::lose) 
	{
		DrawText("You have died. At least you took as many as possible down with you...", 20, int(m_window_size.y * 0.5), 20, m_text_colour);
	}
